...
export class RoomService {
  ...
  async create(createRoomDto: CreateRoomDto) {
    try {
        createRoomDto.image === '' ? createRoomDto.image = 'https://firebasestorage.googleapis.com/v0/b/chat-app-9ab62.appspot.com/o/files%2Fgroup.png?alt=media&token=ce7f4407-df77-4675-9f10-8dc1f7d83869' : createRoomDto.image = createRoomDto.image
      ...
      const newRoom = await this.Room.create({...createRoomDto, name: name, type: 'p2p'});
      ...
      throw new HttpException('Room was created before, or data is incomplete or wrong formatted', 400);
    }
  }
  ...
  async createGroup(createRoomDto: CreateRoomDto) {
    try {
        createRoomDto.image === '' ? createRoomDto.image = 'https://firebasestorage.googleapis.com/v0/b/chat-app-9ab62.appspot.com/o/files%2Fgroup.png?alt=media&token=ce7f4407-df77-4675-9f10-8dc1f7d83869' : createRoomDto.image = createRoomDto.image
      let newRoom: any;
      const users= createRoomDto.users.map( async (user) => await  this.User.findById(user))
        newRoom =  Promise.all(users ).then(async (users) => {
            newRoom = await  this.Room.create({...createRoomDto, type: 'group'});
        ...
        throw new HttpException('Group was created before, or data is incomplete or wrong formatted', 400);
    }
  }
  ...
  async update(id: string, updateRoomDto: UpdateRoomDto){
    try {
      ...
      throw new HttpException('Room does not exists, or incorrect data', 401);
    }
  ...
  async addMessage(id: string, createMessageDto: CreateMessageDto){
    try {
      const room = await this.Room.findById(id);
      const newMessage = {...createMessageDto, createdAt: JSON.stringify(new Date())}
  ...
  async updateSeenMessages(otheUserId: string, token: string, roomId: string){
    try {
